...
import { addPipesAtLocation } from '../../utils/gameHelper';
...
const Physics = (entities, { touches, time, dispatch }) => {
  ...
  touches.filter(t => t.type === "press").forEach(t => {
    if (!hadTouches) {
      if (world.gravity.y === 0.0) {
        ...
        addPipesAtLocation((Constants.MAX_WIDTH * 2) - (Constants.PIPE_WIDTH / 2), world, entities, pipes);
      }
  ...
  Object.keys(entities).forEach(key => {
    if (key.indexOf("pipe") === 0 && entities.hasOwnProperty(key)) {
      Matter.Body.translate(entities[key].body, { x: Platform.OS === 'ios' ? -8 : -10, y: 0 });
